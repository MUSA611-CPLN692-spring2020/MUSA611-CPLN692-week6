...
var makeSchool = function(name, principal, lat, lng) { //add back later: hasSpecial, hasBilingual, color
  var school = {
    "Name" : name,
    "principal" : principal,
    "lat" : lat,
    "lng": lng, // add back in later:     "hasSpecial" : hasSpecial, "hasBilingual" : hasBilingual, "color" : color,
  };
  return school;
};
...
$('button').click(function(e) {
  ...
  makeSchool.principal = $('#text-input2').val();
  console.log("principal", makeSchool.principal);
  makeSchool.lat  = $('#numeric-input1').val();
  console.log("LAT", makeSchool.lat);
  makeSchool.lng = $('#numeric-input2').val();
  console.log("LNG", makeSchool.lng);
  L.circleMarker([makeSchool.lat, makeSchool.lng]).addTo(map);
});
