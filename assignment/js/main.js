...
//Task 3
var newobject = function (url, user_lat, user_lng){
  return {
  'url': url,
  'LAT': user_lat,
  'LNG': user_lng
...
var makeMarkers_new = function(list){
  return _.map(list, function (obj) {
    return L.marker([obj[user_lat], obj[user_lng]])
})
};
...
$('button').click(function() {
  url = $('#text-input1').val();
  user_lat = $('#text-input2').val();
  user_lng = $('#text-input3').val();

  var userinput = newobject(url, user_lat, user_lng);
  console.log(userinput.url, userinput[user_lat], userinput[user_lng]);

  var userData = $.ajax(url);
  userData.done(function(data) {
    var parsed = parseData(data);
    var new_markers = makeMarkers_new(parsed);
    plotMarkers(new_markers);
  });
